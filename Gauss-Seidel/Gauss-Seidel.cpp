...
float gauss_seidel(float matrix[], float aux[], int component);
...
int main(){
    ...
    do{
        for(int i = 0; i < n; i++){
            ...
            x[i] = (1/a[i][i]) * (b[i] - gauss_seidel(aux, x_prev, i));
            //getting error of each variable
            error[i] = get_error(x, x_prev, i);
            //gauss-seidel rule x_prev is now x
            x_prev[i] = x[i];
        }
...
float get_error(float vector1[], float vector2[], int component){
    ...
    error = 100 * fabs((vector1[component] - vector2[component]) / vector1[component]);
    return error;
}
